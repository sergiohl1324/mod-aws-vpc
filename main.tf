...
resource "aws_vpc" "this" {
  ...
  tags = merge(tomap({ Name = format("%s", local.env_name) }), var.tags, var.vpc_tags)
}
...
resource "aws_vpc_dhcp_options" "this" {
  ...
  tags = merge(tomap({ Name = format("%s", local.env_name) }), var.tags, var.dhcp_options_tags)
}
...
resource "aws_internet_gateway" "this" {
  ...
  tags = merge(tomap({ Name = format("%s", local.env_name) }), var.tags, var.igw_tags)
}
...
resource "aws_route_table" "public" {
  ...
  tags = merge(tomap({ Name = format("%s-${var.public_subnet_suffix}", local.env_name) }), var.tags, var.public_route_table_tags)
}
...
resource "aws_route_table" "private" {
  ...
  tags = merge(tomap({ Name = (var.single_nat_gateway ? "${local.env_name}-${var.private_subnet_suffix}" : format("%s-${var.private_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1"))) }), var.tags, var.private_route_table_tags)
...
resource "aws_route_table" "database" {
  ...
  tags = merge(var.tags, var.database_route_table_tags, tomap({ Name = "${local.env_name}-${var.database_subnet_suffix}" }))
}
...
resource "aws_route_table" "redshift" {
  ...
  tags = merge(var.tags, var.redshift_route_table_tags, tomap({ Name = "${local.env_name}-${var.redshift_subnet_suffix}" }))
}
...
resource "aws_route_table" "elasticache" {
  ...
  tags = merge(var.tags, var.elasticache_route_table_tags, tomap({ Name = "${local.env_name}-${var.elasticache_subnet_suffix}" }))
}
...
resource "aws_route_table" "eks" {
  ...
  tags = merge(tomap({ Name = (var.single_nat_gateway ? "${local.env_name}-${var.eks_subnet_suffix}" : format("%s-${var.eks_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1"))) }), var.tags, var.eks_route_table_tags)
}

resource "aws_route" "eks_nat_gateway" {
  count                  = var.create_vpc && var.create_eks_subnet_route_table && length(var.eks_subnets) > 0 && var.create_eks_nat_gateway_route && var.enable_nat_gateway ? length(var.eks_subnets) : 0
...
resource "aws_route_table" "secondary_eks" {
  count = var.create_vpc && var.create_eks_subnet_route_table && length(var.eks_secondary_subnets) > 0 ? length(var.eks_secondary_subnets) : 0
...
resource "aws_route" "secondary_eks_nat_gateway" {
  count                  = var.create_vpc && var.create_eks_subnet_route_table && length(var.eks_secondary_subnets) > 0 && var.create_eks_nat_gateway_route && var.enable_nat_gateway ? length(var.eks_secondary_subnets) : 0
  route_table_id         = element(aws_route_table.secondary_eks.*.id, count.index)
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(aws_nat_gateway.this.*.id, count.index)
...
resource "aws_route_table" "intra" {
  ...
  tags = merge(tomap({ Name = "${local.env_name}-intra" }), var.tags, var.intra_route_table_tags)
}
...
resource "aws_subnet" "public" {
  ...
  vpc_id                  = local.vpc_id
  cidr_block              = element(concat(var.public_subnets, tolist([""])), count.index)
  availability_zone       = element(var.azs, count.index)
  map_public_ip_on_launch = var.map_public_ip_on_launch

  tags = merge(tomap({ Name = format("%s-${var.public_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.public_subnet_tags)
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge(tomap({ Name = format("%s-${var.private_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.private_subnet_tags)
}
...
resource "aws_subnet" "database" {
  ...
  tags = merge(tomap({ Name = format("%s-${var.database_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.database_subnet_tags)
}
...
resource "aws_subnet" "redshift" {
  ...
  tags = merge(tomap({ Name = format("%s-${var.redshift_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.redshift_subnet_tags)
}
...
resource "aws_subnet" "elasticache" {
  ...
  tags = merge(tomap({ Name = format("%s-${var.elasticache_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.elasticache_subnet_tags)
}
...
resource "aws_subnet" "intra" {
  ...
  tags = merge(tomap({ Name = format("%s-intra-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.intra_subnet_tags)
}
...
resource "aws_subnet" "eks" {
  ...
  tags = merge(tomap({ Name = format("%s-${var.eks_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.eks_subnet_tags)
}


resource "aws_subnet" "secondary_eks" {
  count = var.create_vpc && length(var.eks_secondary_subnets) > 0 ? length(var.eks_secondary_subnets) : 0

  vpc_id            = local.vpc_id
  cidr_block        = var.eks_secondary_subnets[count.index]
  availability_zone = element(var.azs, count.index)

  tags = merge(tomap({ Name = format("%s-${var.eks_subnet_suffix}-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.eks_secondary_subnet_tags)
}
...
resource "aws_nat_gateway" "this" {
  ...
  tags = merge(tomap({ Name = format("%s-az-%s", local.env_name, replace(element(var.azs, count.index), "/.*(.)/", "$1")) }), var.tags, var.nat_gateway_tags)
...
resource "aws_vpc_endpoint" "s3" {
  count = var.create_vpc && var.enable_s3_endpoint ? local.s3_endpoint_count : 0
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  ...
  vpc_endpoint_id = aws_vpc_endpoint.s3[var.one_s3_endpoint_per_az ? count.index : 0].id
  route_table_id  = element(aws_route_table.private.*.id, count.index)
}
...
resource "aws_route_table_association" "eks" {
  ...
  route_table_id = element(aws_route_table.eks.*.id, (var.single_nat_gateway ? 0 : count.index))
}

resource "aws_route_table_association" "eks_secondary" {
  count = var.create_vpc && length(var.eks_secondary_subnets) > 0 ? length(var.eks_secondary_subnets) : 0

  subnet_id      = element(aws_subnet.secondary_eks.*.id, count.index)
  route_table_id = element(aws_route_table.secondary_eks.*.id, (var.single_nat_gateway ? 0 : count.index))
}
...
resource "aws_vpn_gateway" "this" {
  ...
  tags = merge(tomap({ Name = format("%s", local.env_name) }), var.tags, var.vpn_gateway_tags)
}
