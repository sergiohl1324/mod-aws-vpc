...
variable "eks_secondary_subnets" {
  type        = list(any)
  description = "A list of secondary eks subnets"
  default     = []
}
...
variable "one_s3_endpoint_per_az" {
  description = "Should be true if you want only one VPC Endpoint per availability zone. Requires `var.azs` to be set, and the number of `private_subnets` created to be greater than or equal to the number of availability zones specified in `var.azs`."
  default     = false
}
...
variable "eks_secondary_subnet_tags" {
  description = "Additional tags for the secondary eks subnets"
  default     = {}
}
