...
output "database_subnet_group" {
  ...
  value       = element(concat(aws_db_subnet_group.database.*.id, tolist([""])), 0)
}
...
output "redshift_subnet_group" {
  ...
  value       = element(concat(aws_redshift_subnet_group.redshift.*.id, tolist([""])), 0)
}
...
output "elasticache_subnet_group" {
  ...
  value       = element(concat(aws_elasticache_subnet_group.elasticache.*.id, tolist([""])), 0)
}

output "elasticache_subnet_group_name" {
  description = "Name of elasticache subnet group"
  value       = element(concat(aws_elasticache_subnet_group.elasticache.*.name, tolist([""])), 0)
}
...
output "vgw_id" {
  ...
  value       = element(concat(aws_vpn_gateway.this.*.id, aws_vpn_gateway_attachment.this.*.vpn_gateway_id, tolist([""])), 0)
}
...
output "default_vpc_cidr_block" {
  description = "The CIDR block of the VPC"
  value       = element(concat(aws_default_vpc.this.*.cidr_block, tolist([""])), 0)
}

output "default_vpc_default_security_group_id" {
  description = "The ID of the security group created by default on VPC creation"
  value       = element(concat(aws_default_vpc.this.*.default_security_group_id, tolist([""])), 0)
}

output "default_vpc_default_network_acl_id" {
  description = "The ID of the default network ACL"
  value       = element(concat(aws_default_vpc.this.*.default_network_acl_id, tolist([""])), 0)
}

output "default_vpc_default_route_table_id" {
  description = "The ID of the default route table"
  value       = element(concat(aws_default_vpc.this.*.default_route_table_id, tolist([""])), 0)
}

output "default_vpc_instance_tenancy" {
  description = "Tenancy of instances spin up within VPC"
  value       = element(concat(aws_default_vpc.this.*.instance_tenancy, tolist([""])), 0)
}

output "default_vpc_enable_dns_support" {
  description = "Whether or not the VPC has DNS support"
  value       = element(concat(aws_default_vpc.this.*.enable_dns_support, tolist([""])), 0)
}

output "default_vpc_enable_dns_hostnames" {
  description = "Whether or not the VPC has DNS hostname support"
  value       = element(concat(aws_default_vpc.this.*.enable_dns_hostnames, tolist([""])), 0)
}

output "default_vpc_main_route_table_id" {
  description = "The ID of the main route table associated with this VPC"
  value       = element(concat(aws_default_vpc.this.*.main_route_table_id, tolist([""])), 0)
}
...
output "vpc_endpoint_dynamodb_pl_id" {
  ...
  value       = element(concat(aws_vpc_endpoint.dynamodb.*.prefix_list_id, tolist([""])), 0)
}

output "vpc_endpoint_ssm_id" {
  description = "The ID of VPC endpoint for SSM"
  value       = element(concat(aws_vpc_endpoint.ssm.*.id, tolist([""])), 0)
}
...
output "vpc_endpoint_ec2_id" {
  ...
  value       = element(concat(aws_vpc_endpoint.ec2.*.id, tolist([""])), 0)
}
